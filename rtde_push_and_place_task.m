...
p_push_end = g_start(1:3,4) + cfg.pushDist * (pushDir+0.01);
...
% segB = rtde_rr_move_to_pose(ur, (g_start+(-0.4,0,0)), cfg, "return_to_start");
R_ref = g_start(1:3,1:3);          % 保持姿态不变
p_new = g_start(1:3,4) + [-0.04; 0; 0];  % 世界坐标系 x 方向平移 -0.4m
g_new = rtde_make_pose(R_ref, p_new);

segB = rtde_rr_move_to_pose(ur, g_new, cfg, "return_to_start");
...
% Segment C: Lift to position (1) above (23cm) - safe intermediate point
p_start_above = g_start(1:3,4);
p_start_above(2) = g_start(2,4) - 0.2; % 23cm above position (1)
...
p_target2_above = g_start(1:3,4) + 0.05 * pushDir;
p_target2_above(2) = g_start(2,4)- 0.2; % 10cm above, same height
...
% Segment E: Lower to position (2) at contact height
 p_target2 = g_start(1:3,4) + (cfg.cubeSide +cfg.pushDist) * pushDir;
 p_target2(2) = g_start(2,4)-0.05; % Same height as the start position
...
p_push_back_end = g_contact2(1:3,4) - cfg.pushDist * pushDir*3.7; % 6cm = 2 * 3cm
