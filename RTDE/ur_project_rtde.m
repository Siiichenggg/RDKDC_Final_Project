...
DT = 0.08;                     % RR step time (s) -- larger for smoother hardware motion
MAX_STEPS = 800;               % max iterations per RR segment
POS_TOL = 4e-3;                % 4 mm
ROT_TOL = 2e-2;                % rad
Kp_pos = 1.2;                  % lower gains to reduce jerk
Kp_rot = 1.0;
...
try
    ...
    % Lift straight up from end (keep orientation)
    g_lift_from_end = g_end_actual;
    g_lift_from_end(1:3,1:3) = R_task;
