"""Inverse kinematics utilities for UR5/UR5e robot arms.

This module provides analytical inverse kinematics solutions for UR5 and UR5e
robots using Denavit-Hartenberg parameters. It includes helper functions for
DH transformations and rotation matrices.
"""
...
def urInvKin(gd, robot_type='ur5e'):
    ...
    Args:
        gd (numpy.ndarray): Desired 4x4 homogeneous transformation matrix.
        robot_type (str): Robot type, either 'ur5' or 'ur5e'. Defaults to 'ur5e'.

    Returns:
        numpy.ndarray: Array of valid joint angle solutions (6 x N), where N is
                       the number of valid solutions.

    Raises:
        ValueError: If gd is not a 4x4 matrix.
    """
    if gd.shape != (4, 4):
        raise ValueError("urInvKin function: Input 'gd' must be a 4x4 matrix.")

    # Initialize with NaNs
    theta = np.full((6, 8), np.nan)
    ...
    for c in [0, 2, 4, 6]:
        ...
        sin_t5 = np.sin(t5)
        if abs(sin_t5) < 1e-6:
            # At singularity, choose one solution
            theta[5, c:c+2] = 0
        else:
